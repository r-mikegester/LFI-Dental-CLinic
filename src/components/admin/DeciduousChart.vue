...
onMounted(async () => {
  originalDataUrl.value = await getDeciduousChart(props.patientUid)
...
const onSaveCanvas = async () => {
  if (!isSaveInProgress.value) {
    ...
    await saveDeciduousChart(props.patientUid, dataURL)
...
<template>
  <div class="w-[16rem] mx-auto">
    ...
    <canvas
      ...
      ref="canvas"
      class="h-96 w-full border border-black bg-contain bg-no-repeat bg-center"
      :style="{
        backgroundImage: `url(${deciduousChart})`,
      }"
    ></canvas>
  </div>
</template>
