...
import getGuardianSignature from "../../composables/firestore/signature-fields/getGuardianSignature";
import setGuardianSignature from "../../composables/firestore/signature-fields/setGuardianSignature";
...
onMounted(async () => {
  originalDataUrl.value = await getGuardianSignature(props.patientUid);
...
const onSaveCanvas = async () => {
  if (!isSaveInProgress.value) {
    ...
    await setGuardianSignature(props.patientUid, dataURL);
...
<template>
  <div class="w-[24rem] mx-auto">
    ...
    <canvas
      ...
      ref="canvas"
      class="h-24 w-full border border-black"
    ></canvas>
  </div>
</template>
